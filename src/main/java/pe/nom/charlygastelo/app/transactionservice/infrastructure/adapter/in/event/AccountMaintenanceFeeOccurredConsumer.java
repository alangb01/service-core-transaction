...
import pe.nom.charlygastelo.app.shared.avro.dto.AccountMaintenanceFeeOccurredEvent;
import pe.nom.charlygastelo.app.transactionservice.application.usecase.TransactionProgressUseCase;

@Component
@RequiredArgsConstructor
@Slf4j
public class AccountMaintenanceFeeOccurredConsumer {

    private final TransactionProgressUseCase progressUseCase;

    @KafkaListener(topics = "${topic.account-maintenance-fee-occurred}", groupId = "transaction-service")
    public void consume(AccountMaintenanceFeeOccurredEvent event) {
